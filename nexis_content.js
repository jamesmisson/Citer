...
function parseCitation() {
  ...
  citObject.xmlString = `<q d="nex"><cit><d>${citObject.d}</d><bibMain><w>${citObject.w.replace('(', '<place>').replace(')', '</place>')}</w><di>${citObject.di}</di></bibMain></cit><qt>${citObject.qt}</qt><q>`
